...
*/
vsi_status vnn_PreProcessMobilenetv1Tfite_ImageProcess
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
    )
{
    ...
    {
        printf("vnn_PreProcessMobilenetv1Tfite_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
...
vsi_status vnn_PreProcessMobilenetv1Tfite
    (
    vsi_nn_graph_t *graph,
    const char **inputs,
    uint32_t input_num
